...
struct MoviesView: View {
    ...
    @ObservedObject private var presenter: MoviesVMPresenter
    private let useCase: MoviesUseCaseInput
    
    var body: some View {
        NavigationView {
            ZStack {
                VStack {
                    SearchBar(text: self.$presenter.state.model.searchText.onChange({ text in
                        self.useCase.getFilteredMovies(text: text,
                                                       page: 1)
                    }))
                    if !self.presenter.state.model.searchText.isEmpty {
                        List {
                            ForEach(self.presenter.state.model.movies) { movie in
                        ...
                        List {
                            LazyVStack {
                                ForEach(self.presenter.state.model.categories) { category in
                                    Section(header:
    ...
    init(presenter: MoviesVMPresenter = MoviesVMPresenter()) {
        self.presenter = presenter
        self.useCase = MoviesVMUseCase(
            presenter: presenter,
            middleware: MoviesMiddleware(storeManager: SQLiteStoreManagerMovie(),
                                         networkManager: URLSessionNetworkManager(),
